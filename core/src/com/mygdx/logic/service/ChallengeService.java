...
public class ChallengeService extends Service{
    ...
    public void save(Object object) throws Exception {
        Map map=(Map) object;
        int newId = new JsonParser().parse(repositoryFactory.get("CHALLENGES").get()).getAsJsonArray().size()+1;
        ...
        repositoryFactory.get("CHALLENGES").save(jsonObject);
    }

    @Override
    public List<Map> get() throws Exception {
        List<Map> maps = new ArrayList<>();
        JsonArray mapArray = new JsonParser().parse(repositoryFactory.get("CHALLENGES").get()).getAsJsonArray();

        this.jsonIterator = getJsonIterator(mapArray);

        while(jsonIterator.hasNext()){
            JsonObject jsonObject = jsonIterator.nextJson();
            maps.add((Map)director.buildObject(new MapBuilder()
                    .setMapId(jsonObject.get("mapId").getAsInt())
